...
static PHP_FUNCTION(swoole_substr_json_decode) {
    ...
    if (str_len == 0) {
        zend_argument_value_error(1, "must be not empty");
        RETURN_THROWS();
    }
    if (offset < 0) {
        offset = str_len + offset;
        if (offset < 0) {
            zend_argument_value_error(2, "must be not less than the negative length of the string");
            RETURN_THROWS();
        }
    }
    if ((zend_long) str_len <= offset) {
        zend_argument_value_error(2, "must be less than the length of the string");
        RETURN_THROWS();
    }
